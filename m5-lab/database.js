...
export function saveMenuItems(menuItems) {
  ...
  db.transaction((tx) => {
    ...
    //  const array = menuItems[i];
    //  tx.executeSql('INSERT INTO menuitems (id, uuid, title, price, category) VALUES (?,?,?,?,?)', [array.id,array.uuid,array.title,array.price,array.category])
    //}
    //menuItems.map((menuItem) => {
    //  tx.executeSql('insert into menu (id, uuid, title, price, category) values (?, ?, ?, ?, ?)', [menuItem.id, menuItem.id, menuItem.title, menuItem.price, menuItem.category.title]);
    //})
    tx.executeSql(
...
export async function filterByQueryAndCategories(query, activeCategories) {
  ...
  return new Promise((resolve, reject) => {
    db.transaction((tx) => {
      // syntax here is: query, arguments for the statement [], a function to handle the result,
      // an error handler
      tx.executeSql(`select * from menuitems where title like '%${query}%' and category in 
